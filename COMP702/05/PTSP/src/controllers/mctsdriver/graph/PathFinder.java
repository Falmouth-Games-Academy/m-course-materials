        ...
        while(!toEvaluateSet.isEmpty())
        {
            ...
            for(Long edgeId:currentNode.getEdgesFromNode())
            {
                ...
                if(!evaluatedSet.contains(connected))
                {
                    ...
                    if (DA.m_points.size() >= 2)
                    {
	                    Vector2d connectedDirection = new Vector2d(connected.x() - currentNode.x(), connected.y() - currentNode.y());
	                    connectedDirection.normalise();
	                    Node lastNodeInDA = m_graph.getNode(DA.m_points.get(DA.m_points.size() - 2));
	                    Vector2d daDirection = new Vector2d(currentNode.x() - lastNodeInDA.x(), currentNode.y() - lastNodeInDA.y());
	                    daDirection.normalise();
	                    
	                    dA1 += (1 - connectedDirection.dot(daDirection)) * m_graph.PATH_TURN_COST;
                    }
