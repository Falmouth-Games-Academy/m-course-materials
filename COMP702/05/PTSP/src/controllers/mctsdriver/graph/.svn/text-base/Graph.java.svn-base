package controllers.mctsdriver.graph;
...
public class Graph
{
	/** The cost of a 90 degree turn in the path.
	 *  A 180 degree turn is worth twice this.
	 *  A 45 degree turn is worth 1-cos(45 degrees) = 0.292 times this. */
	public double PATH_TURN_COST = 100;
	
	/** Whether to include edges in the graph for knight's moves, as well as the usual king's moves. */
	public boolean INCLUDE_KNIGHT_MOVES = true;
    ...
    public Graph(Game a_game)
    {
        ...
        for(int i = 0; i < m_map.getMapChar().length; i+=GRANULARITY)
        {
            ...
            {
                if(!m_map.isObstacle(i,j) && !isObstacleClose(i, j, 4) && !a_game.getShip().checkCollisionInPosition(new Vector2d(i,j)))
                {
                    addNode(i,j);
                }
            }
    ...
    private void connect(Node n, int dx, int dy)
    {
    	int x = n.x();
        int y = n.y();
        
        if (x+dx >= 0 && x+dx < m_map.getMapWidth() && y+dy >= 0 && y+dy < m_map.getMapHeight())
        {
        	int nodeID = existsNode(x+dx, y+dy);
            if(nodeID != -1 && m_map.checkObsFree(x, y, x+dx, y+dy))
                addEdge(n.id(), nodeID, Math.sqrt(dx*dx + dy*dy));
        }
    }
    ...
    private void eightWayConnectivity(Node n)
    {
    	for (int dx=-1; dx<=+1; dx++)
    		for (int dy=-1; dy<=+1; dy++)
    			if (dx!=0 || dy!=0)
    				connect(n, dx * GRANULARITY, dy * GRANULARITY);
    	
    	if (INCLUDE_KNIGHT_MOVES)
    	{
	    	for (int dx=-1; dx<=+1; dx+=2)
	    		for (int dy=-1; dy<=+1; dy+=2)
	    		{
	    			connect(n, dx * GRANULARITY, 2 * dy * GRANULARITY);
	    			connect(n, 2 * dx * GRANULARITY, dy * GRANULARITY);
	    		}
    	}
    }
                ...
                if(x >= 0 && y >= 0 && x < m_map.getMapWidth() && y < m_map.getMapHeight())
                {
                    ...
                    if(nodeID != -1)
                    {
                        ...
                        if(distance < bestDistance)
                        {
                            if(a_checkObsFree)
                            {
                                if(m_map.checkObsFree(n.x(), n.y(), xPos, yPos))
                                {
                                    bestNode = n;
